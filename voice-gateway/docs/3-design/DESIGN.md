...
- `02 最小闭环` 已完成真实音箱端到端打通。
- 当前主链路为：音箱 record PCM -> Mac Mini VAD/ASR -> Hermes -> Edge TTS MP3 URL -> 音箱 `miplayer`。
- 默认唤醒词为“你好”；连接成功后播放缓存短提示“已连接”；唤醒后随机播放“我在 / 在 / 诶”作为确认提示。
- 运行入口为 `voice-gateway/scripts/run-voice-gateway-minimal.sh`。
- `03 连续对话` 首版已实现：回答结束后进入免唤醒追问窗口，并向 Hermes 传递最近 10 轮历史。
- 下一阶段可继续实测连续对话体验，随后进入 `04 自然打断`。
